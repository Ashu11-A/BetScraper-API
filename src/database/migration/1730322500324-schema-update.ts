...
export class SchemaUpdate1730322500324 implements MigrationInterface {
  name = 'SchemaUpdate1730322500324'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `users` ADD `lastExecutedAt` timestamp NULL')
    await queryRunner.query('ALTER TABLE `users` DROP COLUMN `nextExecutionAt`')
    await queryRunner.query('ALTER TABLE `users` ADD `nextExecutionAt` timestamp NULL')
    ...
    await queryRunner.query('ALTER TABLE `tasks` ADD `finishedAt` timestamp NULL')
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `tasks` DROP COLUMN `finishedAt`')
    await queryRunner.query('ALTER TABLE `tasks` ADD `finishedAt` date NULL DEFAULT \'NULL\'')
    await queryRunner.query('ALTER TABLE `tasks` CHANGE `duration` `duration` int NULL DEFAULT \'NULL\'')
    await queryRunner.query('ALTER TABLE `tasks` ADD CONSTRAINT `FK_985b3a08f36589f12e23647c07d` FOREIGN KEY (`cronId`) REFERENCES `crons`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION')
    await queryRunner.query('ALTER TABLE `tasks` ADD CONSTRAINT `FK_166bd96559cb38595d392f75a35` FOREIGN KEY (`userId`) REFERENCES `users`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION')
    await queryRunner.query('ALTER TABLE `tasks` ADD CONSTRAINT `FK_e4bcf0bf21c7ceb459f67c42193` FOREIGN KEY (`betId`) REFERENCES `bets`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION')
    await queryRunner.query('ALTER TABLE `users` DROP COLUMN `nextExecutionAt`')
    await queryRunner.query('ALTER TABLE `users` ADD `nextExecutionAt` date NULL DEFAULT \'NULL\'')
    await queryRunner.query('ALTER TABLE `users` DROP COLUMN `lastExecutedAt`')
    await queryRunner.query('ALTER TABLE `users` ADD `lastExecutedAt` date NULL DEFAULT \'NULL\'')
