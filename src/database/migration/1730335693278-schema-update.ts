...
export class SchemaUpdate1730335693278 implements MigrationInterface {
  name = 'SchemaUpdate1730335693278'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('ALTER TABLE `tasks` ADD `errorMessage` text NULL')
    await queryRunner.query('ALTER TABLE `tasks` ADD `scheduledAt` timestamp NULL')
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `infraction` ADD CONSTRAINT `FK_5ffd257c3b15678a6f17ba7207f` FOREIGN KEY (`betsId`) REFERENCES `bets`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION')
    await queryRunner.query('ALTER TABLE `tasks` DROP COLUMN `scheduledAt`')
    await queryRunner.query('ALTER TABLE `tasks` DROP COLUMN `errorMessage`')
  }
