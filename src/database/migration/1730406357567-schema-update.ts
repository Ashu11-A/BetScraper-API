...
export class SchemaUpdate1730406357567 implements MigrationInterface {
    name = 'SchemaUpdate1730406357567'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`crons\` (\`id\` int NOT NULL AUTO_INCREMENT, \`expression\` varchar(255) NOT NULL, \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` int NOT NULL AUTO_INCREMENT, \`uuid\` varchar(36) NOT NULL, \`name\` text NOT NULL, \`username\` text NOT NULL, \`email\` varchar(255) NOT NULL, \`language\` varchar(255) NOT NULL, \`password\` text NOT NULL, \`lastExecutedAt\` timestamp NULL, \`nextExecutionAt\` timestamp NULL, \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
